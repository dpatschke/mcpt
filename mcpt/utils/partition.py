...
def set_binning_variables(data, nbins, target='cpu'):
    ...
    # If nbins is None, then the data being passed in is
    # discrete and the value of nbins needs to be the
    # largest number of levels among all the variables that
    # are part of the data set
    if nbins is None:
        nbins_list = [len(np.unique(data[i])) for i in range(ncols)]
        nbins = np.max(nbins_list)
        # The following two objects won't be needed with 'masters_cut'
        # but will be used to flag whether discrete or not
        bounds = np.ones((ncols, nbins), dtype=np.int32)
        bin_end = np.ones((ncols, nbins), dtype=np.int32)
    else:
        bounds = np.zeros((ncols, nbins), dtype=np.int32)
        bin_end = np.zeros((ncols, nbins), dtype=np.int32)
    
    n_bins = np.repeat(nbins, ncols).astype(np.int32)
...
def masters_cut_cpu(data_copy, n_bins_array, bounds, bins,
    ...
    # If 'bounds' is all zeros, then the variables being
    # passed in are numeric and need to be binned. Otherwise,
    # the variables are discrete and nothing needs to be done.
    if np.max(bounds) == 0:
        ...
        for i in numba.prange(data_copy.shape[0]):
            bins[i] = data_copy[i]
            n_bins_array[i] = len(np.unique(bins[i]))
